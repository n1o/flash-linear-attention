...
from fla.ops.qgdn2 import chunk_qgdn2, fused_recurrent_qgdn2
...
class QGatedDeltaNet2(nn.Module):
    r"""
    Q-Gated DeltaNet 2 (QGDN2) layer.

    QGDN2 extends GDN2 by using a query-conditioned erase direction
    ``x_t = k_t + lambda_t * q_t`` while keeping the write direction at
    ``k_t``. The matrix-state recurrence is

        S_t = (I - k_t (b_t * x_t)^T) Diag(exp(g_t)) S_{t-1}
              + k_t (w_t * v_t)^T

    where ``lambda_t`` is a learned per-head scalar in ``[0, 1]``.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # QGDN2 channel-wise gates: b on the K axis, w on the V axis.
        self.b_proj = nn.Linear(hidden_size, self.key_dim, bias=False)
        self.w_proj = nn.Linear(hidden_size, self.value_dim, bias=False)
        self.lamb_proj = nn.Linear(hidden_size, self.num_heads, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # QGDN2 channel-wise gates, both squashed to [0, 1].
        b = self.b_proj(hidden_states).sigmoid()
        w = self.w_proj(hidden_states).sigmoid()
        lq = torch.sigmoid(self.lamb_proj(hidden_states) - self.lamb_bias)
        ...
        if self.num_v_heads > self.num_heads:
            ...
            lq = repeat(lq, "... h -> ... (h g)", g=self.num_v_heads // self.num_heads)
        ...
        if mode == "chunk":
            ...
            o, recurrent_state = fused_recurrent_qgdn2(
                q=q,
                k=k,
                v=v,
                g=g,
