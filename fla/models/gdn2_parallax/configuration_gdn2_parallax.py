...
class GDN2ParallaxConfig(PretrainedConfig):
    model_type = 'gdn2_parallax'
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if attn_mode not in ("chunk", "recurrent", "fused_recurrent", "reference"):
            raise ValueError(
                f"GDN2Parallax only supports `chunk`, `recurrent`, `fused_recurrent`, or `reference` mode, got `{attn_mode}`."
            )
