...
class GDN2ParallaxBlock(GradientCheckpointingLayer):

    def __init__(self, config: GDN2ParallaxConfig, layer_idx: int):
        super().__init__()
        ...
        if config.attn is not None and layer_idx in config.attn['layers']:
            ...
            self.attn = GDN2Parallax(
                ...
                parallax_mean_decay=config.parallax_mean_decay,
                parallax_rho_max=config.parallax_rho_max,
                layer_idx=layer_idx,
            )
        self.mlp_norm = (RMSNorm if config.fuse_norm else nn.RMSNorm)(config.hidden_size, eps=config.norm_eps)
        self.mlp = GDN2ParallaxMLP(
            hidden_size=config.hidden_size,
...
class GDN2ParallaxPreTrainedModel(PreTrainedModel):

    config_class = GDN2ParallaxConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['GDN2ParallaxBlock']
    ...
    def _init_weights(
        ...
        if isinstance(module, GDN2Parallax) and next(module.parameters()).device.type != 'meta':
            with torch.no_grad():
                ...
                nn.init.zeros_(module.parallax_gamma)
...
class GDN2ParallaxModel(GDN2ParallaxPreTrainedModel):

    def __init__(self, config: GDN2ParallaxConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([GDN2ParallaxBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`GDN2ParallaxModel` does not `output_attentions` now, setting it to `False`.")
...
class GDN2ParallaxForCausalLM(GDN2ParallaxPreTrainedModel, FLAGenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = GDN2ParallaxModel(config)
        self.vocab_size = config.vocab_size
