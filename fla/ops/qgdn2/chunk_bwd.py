...
r"""Backward pipeline for Q-GDN-2 chunkwise training.

This file owns the Q-GDN-2-specific WY-aware backward kernel and the end-to-end
...
Q-GDN-2 changes the erase direction from ``k`` to ``x = k + lq * q``. In the
backward this means:

1. The ``dA`` from the WY erase side changes from ``dW @ (b * exp(g) * k)^T``
   to ``dW @ (b * exp(g) * x)^T``.
2. ``db`` changes from ``dAkk_bwd * k`` to ``dAkk_bwd * x``.
3. A new ``dlq`` gradient: ``dAkk_bwd * b * exp(g) * q`` summed over the K
   dimension.
4. ``dq`` gets an additional contribution from the erase side:
   ``dAkk_bwd * b * exp(g) * lq``.
5. ``dk`` from the erase side is unchanged (since ``dx/dk = 1``).
6. The ``dg`` computation uses ``x * exp(g)`` instead of ``k * exp(g)``.

When ``lq`` is ``None`` the kernel reduces exactly to GDN-2 with ``x = k``.

The crux of the WY backward is the vector-Jacobian product through
...
from fla.ops.qgdn2.chunk_intra import chunk_gdn2_bwd_intra
from fla.ops.qgdn2.wy_fast import recompute_w_u_fwd_gdn2
...
@triton.heuristics({
    ...
    'USE_Q_DELTA': lambda args: args['lq'] is not None,
})
@fla_cache_autotune(
    configs=[triton.Config({'BK': 32, 'BV': 32}, num_warps=4, num_stages=2)],
...
def chunk_gdn2_bwd_kernel_wy_dqkg_fused(
    ...
    if USE_Q_DELTA:
        b_dlq = tl.zeros([BT], dtype=tl.float32)
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        if USE_Q_DELTA:
            # Erase direction x = k + lq * q: load q and lq before building b_kg.
            p_q = tl.make_block_ptr(q, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            b_q = tl.load(p_q, boundary_check=(0, 1))
            # lq is [B, T, H] -- one scalar per head per token.
            p_lq = lq + (bos + o_t) * H + i_h
            b_lq = tl.load(p_lq, mask=m_t, other=0).to(tl.float32)
            b_x = b_k.to(tl.float32) + b_lq[:, None] * b_q.to(tl.float32)
            # x * exp(g) replaces k * exp(g) on the erase side.
            b_kg = b_x * b_gk_exp
        else:
            b_kg = b_k * b_gk_exp

        b_dw_flow = -b_dw_flow.to(b_A.dtype)
        # dA gets (b * exp(gk) * x) on the key side - the Q-GDN-2 erase twist.
        ...
        # db uses x * exp(g) instead of k * exp(g).
        b_db_partial = b_dkgb * b_kg
        tl.store(p_db, b_db_partial.to(p_db.dtype.element_ty), boundary_check=(0, 1))

        if USE_Q_DELTA:
            # dlq: dAkk_bwd * b * exp(g) * q, accumulated across K-blocks.
            b_dlq += tl.sum(b_dkgb * b_gb * b_q.to(tl.float32), axis=1)
            # Extra dq from erase side (dx/dq = lq).
            b_dq = b_dq + b_dkgb * b_gb * b_lq[:, None]
        else:
            p_q = tl.make_block_ptr(q, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            b_q = tl.load(p_q, boundary_check=(0, 1))
        ...
        # b_dg uses b_kg (now x * exp(g)) on the erase term.
        b_dg = b_q * b_dq - b_kdk + m_last[:, None] * b_dgk + b_kg * b_dkgb * b_b
        # dk from erase = dkgb * exp(g) * b (unchanged: dx/dk = 1).
    ...
    if USE_Q_DELTA:
        p_dlq = dlq + (bos + o_t) * H + i_h
        tl.store(p_dlq, b_dlq.to(dlq.dtype.element_ty), mask=m_t)
...
def chunk_gdn2_bwd_wy_dqkg_fused(
    ...
    """Fused WY backward producing dq, dk, dv, db (K-dim), dw (V-dim), dg, dA, dlq."""
    ...
    dlq = torch.zeros(B, T, H, device=q.device, dtype=torch.float32) if lq is not None else None
    ...
    dv = dv2
    return dq, dk, dv, db, dw, dg, dA, dlq
...
def chunk_gdn2_bwd(
    ...
    """End-to-end Q-GDN-2 backward.

    Returns (dq, dk, dv, db, dw, dg, dlq, dh0, dA_log, dt_bias_grad). ``db`` has
    shape [B, T, H, K] (channel-wise erase gate); ``dw`` has shape [B, T, H, V]
    (channel-wise write gate). ``dlq`` has shape [B, T, H] (per-head query
    feedback strength gradient), or ``None`` when ``lq`` is ``None``.
    """
    ...
    dq, dk, dv, db, dw, dg, dAkk, dlq = chunk_gdn2_bwd_wy_dqkg_fused(
        q=q,
        k=k,
        v=v,
        v_new=v_new,
    ...
    dq, dk, db, dg, dlq_intra = chunk_gdn2_bwd_intra(
        q=q,
        k=k,
        g=g,
        b=b,
    ...
    if dlq is not None and dlq_intra is not None:
        dlq = dlq + dlq_intra
    ...
    return dq, dk, dv, db, dw, dg, dlq, dh0, dA_log, dt_bias_grad
