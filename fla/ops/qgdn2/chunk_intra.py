...
from fla.ops.qgdn2.chunk_intra_token_parallel import chunk_gdn2_fwd_intra_token_parallel
...
def chunk_gdn2_fwd_kernel_intra_sub_chunk(
    ...
    if USE_Q_DELTA:
        p_lq = lq + (bos + o_c) * H + i_h
        b_lq = tl.load(p_lq, mask=m_c, other=0).to(tl.float32)
        b_x = b_k.to(tl.float32) + b_lq[:, None] * b_q.to(tl.float32)
        b_bk = (b_b.to(tl.float32) * b_x).to(b_k.dtype)
    else:
...
@fla_cache_autotune(
    configs=[triton.Config({'BK': 32}, num_warps=4)],
    key=["H", "K", "BC"],
    **autotune_cache_kwargs,
)
@triton.jit(do_not_specialize=['T'])
def chunk_gdn2_fwd_kernel_inter_solve_fused(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        if i_tc1 < T:
            ...
            if USE_Q_DELTA:
                o_tc1_l = i_tc1 + tl.arange(0, BC)
                p_lq1 = lq + (bos + o_tc1_l) * H + i_h
                b_lq1 = tl.load(p_lq1, mask=m_tc1, other=0).to(tl.float32)
                b_bk1 = b_b1 * (b_k1 + b_lq1[:, None] * b_q1)
            else:
                b_bk1 = b_b1 * b_k1
            ...
            if i_tc2 < T:
                ...
                b_qg2 = b_q2 * b_gqn2
                if USE_Q_DELTA:
                    o_tc2_l = i_tc2 + tl.arange(0, BC)
                    p_lq2 = lq + (bos + o_tc2_l) * H + i_h
                    b_lq2 = tl.load(p_lq2, mask=m_tc2, other=0).to(tl.float32)
                    b_bkg2 = (b_b2 * (b_k2 + b_lq2[:, None] * b_q2)) * b_gqn2
                else:
                ...
                if i_tc3 < T:
                    ...
                    b_qg3 = b_q3 * b_gqn3
                    if USE_Q_DELTA:
                        o_tc3_l = i_tc3 + tl.arange(0, BC)
                        p_lq3 = lq + (bos + o_tc3_l) * H + i_h
                        b_lq3 = tl.load(p_lq3, mask=m_tc3, other=0).to(tl.float32)
                        b_bkg3 = (b_b3 * (b_k3 + b_lq3[:, None] * b_q3)) * b_gqn3
                    else:
...
def chunk_gdn2_bwd_kernel_intra(
    ...
    # For Q-GDN-2, the erase direction is x = k + lq * q, so db uses x
    # and we also extract dlq and additional dq from the erase side.
    if USE_Q_DELTA:
        o_c = i_ti + tl.arange(0, BC)
        m_c = o_c < T
        p_lq = lq + (bos + o_c) * H + i_h
        ...
        b_x = b_k.to(tl.float32) + b_lq[:, None] * b_q.to(tl.float32)
        b_db_tile = b_dk2 * b_x
        b_dlq_tile = tl.sum(b_dk2 * b_b.to(tl.float32) * b_q.to(tl.float32), axis=1)
        b_dq2 += b_dk2 * b_b.to(tl.float32) * b_lq[:, None]
        p_dlq = dlq + o_c * H
        tl.atomic_add(p_dlq, b_dlq_tile.to(dlq.dtype.element_ty), mask=m_c)
    else:
        b_db_tile = b_dk2 * b_k
    b_dk2 = b_dk2 * b_b
    ...
    if i_i < NC - 1:
        ...
        for i_j in range(i_i + 1, NC):
            ...
            if USE_Q_DELTA:
                o_jl = i_t * BT + i_j * BC + o_i
                p_lq_j = lq + (bos + o_jl) * H + i_h
                b_lq_j = tl.load(p_lq_j, mask=(o_jl < T), other=0).to(tl.float32)
                b_kbj = (tl.load(p_k, boundary_check=(0, 1)).to(tl.float32) + b_lq_j[:, None] * b_qj.to(tl.float32)) * b_bj.to(tl.float32)
            else:
    ...
    if SAFE_GATE:
        ...
        if USE_Q_DELTA:
            ...
            b_lq = tl.load(p_lq, mask=m_c, other=0).to(tl.float32)
            b_x = b_k.to(tl.float32) + b_lq[:, None] * b_q.to(tl.float32)
            b_kb_exp = b_x * b_b.to(tl.float32) * exp_b_g_diag_kk
        else:
        ...
        for j in range(0, min(BC, T - i_t * BT - i_i * BC)):
            ...
            if USE_Q_DELTA:
                p_lq_j = lq + (bos + i_ti + j) * H + i_h
                b_lq_j = tl.load(p_lq_j, mask=((i_ti + j) < T), other=0).to(tl.float32)
                b_kbj = (b_kj + b_lq_j * b_qj) * b_bj
            else:
                b_kbj = b_kj * b_bj
            b_gkj = tl.load(p_gkj, mask=m_k, other=0).to(tl.float32)
            m_i = o_i[:, None] <= j
...
def chunk_gdn2_fwd_intra(
    ...
    w, u, qg, kg = recompute_w_u_fwd_gdn2(
        ...
        w_gate=w_gate,
        A=Akk,
        q=q if (disable_recompute or lq is not None) else None,
        gk=gk,
        cu_seqlens=cu_seqlens,
        chunk_indices=chunk_indices,
        lq=lq,
...
def chunk_gdn2_bwd_intra(
    ...
    accumulator passed in.  When ``lq`` is provided (Q-GDN-2), also returns
    ``dlq`` (per-head gradient, ``[B, T, H]``).
    """
    B, T, H, K = k.shape
    BT = chunk_size
    BC = min(16, BT)
    ...
    dlq2 = torch.zeros(B, T, H, device=q.device, dtype=torch.float32) if lq is not None else None
