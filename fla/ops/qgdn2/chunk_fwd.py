...
# Forward orchestration for Q-GDN-2 chunkwise training (Phase 4: Chunk Forward).
#
# This is mostly orchestration: it threads the per-head query feedback strength
# ``lq`` through to ``chunk_gdn2_fwd_intra`` so that the erase direction uses
# ``x = k + lq * q`` (unscaled q) instead of ``k``.  When ``lq`` is None the
# pipeline reduces exactly to vanilla GDN-2.  The inter-chunk state recurrence
# and the output composition are unchanged from GDN-2.

import torch
from fla.ops.qgdn2.chunk_intra import chunk_gdn2_fwd_intra
...
def chunk_gdn2_fwd(
    ...
    """Top-level Q-GDN-2 forward pipeline.
    ...
    The pipeline is:
      ...
      2. Build the intra-chunk score matrices (Aqk, Akk_inv) and the WY
         ...
         threading ``lq`` so the erase direction uses ``x = k + lq * q``.
