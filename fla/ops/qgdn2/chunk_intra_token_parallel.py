...
# Token-parallel intra-chunk kernel for Q-GDN-2 (Phase 4: Chunk Forward).
#
...
#
# Difference vs GDN-2: the erase direction uses x = k + lq * q (unscaled q)
# instead of k, so the channel-wise erase gate ``b`` is folded into x before
# the dot product.  When ``lq`` is None the kernel reduces to vanilla GDN-2.
#
...
@fla_cache_autotune(
    configs=[triton.Config({'BH': 1}, num_warps=1)],
    key=['K', 'H'],
...
def chunk_gdn2_fwd_kernel_intra_token_parallel(
    ...
    # For Q-GDN-2 the erase direction is x = k + lq * q (unscaled q), so the
    # gate is folded into x rather than k.  Only the Akk path (which uses the
    # modified b_k) is affected; the Aqk path uses b_q directly and is unchanged.
    if USE_Q_DELTA:
        # lq is [B, T, H] — one scalar per head per token.  Load a [BH] vector
        # for the heads in this group and broadcast across the K dimension.
        p_lq = lq + (bos + i_t) * H + i_hg * BH + o_h
        b_lq = tl.load(p_lq, mask=m_h, other=0.0).to(tl.float32)
...
def chunk_gdn2_fwd_intra_token_parallel(
    ...
    When ``lq`` is provided (Q-GDN-2), the erase direction uses
    ``x = k + lq * q`` (unscaled q) instead of ``k``; the channel-wise erase
    gate ``b`` is then folded into ``x``.  When ``lq`` is None the kernel
    reduces to vanilla GDN-2.
    """
    B, T, H, K = q.shape
