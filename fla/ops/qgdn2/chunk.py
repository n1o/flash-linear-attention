...
r"""
Chunkwise training kernel for query-conditioned GDN2 (QGDN2).

QGDN2 changes GDN2's erase direction from ``k_t`` to
``x_t = k_t + lambda_t * q_t`` while keeping the write direction at ``k_t``.
When ``lambda_t = 0`` it reduces to vanilla GDN2.
"""
...
from fla.ops.qgdn2.chunk_bwd import chunk_gdn2_bwd
from fla.ops.qgdn2.chunk_fwd import chunk_gdn2_fwd
from fla.ops.utils import prepare_chunk_indices
from fla.utils import autocast_custom_bwd, autocast_custom_fwd, input_guard


class ChunkGDN2Function(torch.autograd.Function):
    """Autograd-compatible wrapper around QGDN2 forward / backward."""
    ...
    def forward(
        ...
        ctx.save_for_backward(
            ...
            initial_state, cu_seqlens, chunk_indices, lq,
        )
    ...
    def backward(ctx, do: torch.Tensor, dht: torch.Tensor):
        (q, q_rstd, k, k_rstd, v, g_cumsum, g_input, b, w, A_log, dt_bias,
         Aqk, Akk, w_wy, u_wy, qg, kg, v_new, h,
         initial_state, cu_seqlens, chunk_indices, lq) = ctx.saved_tensors

        dq, dk, dv, db, dw, dg, dlq, dh0, dA_log, dt_bias_grad = chunk_gdn2_bwd(
            q=q,
            k=k,
            v=v,
            b=b,
...
def chunk_gdn2(
    ...
    **kwargs,
    ...
    r"""
    Chunkwise forward for query-conditioned GDN2 (QGDN2).
    ...
    Args:
        ...
        state_v_first: store the recurrent state in ``[V, K]`` layout instead
            of the default ``[K, V]``.
        lq: optional per-head query feedback strength of shape ``[B, T, H]`` (or
            broadcastable). When provided, the erase direction uses
            ``x = k + lq * q`` (unscaled q) instead of ``k``. Computed from
            ``lamb_proj(hidden_states)`` outside the kernel; gradients flow back
            to ``lamb_proj`` via ``dlq``. When ``None`` the pipeline reduces
            exactly to vanilla GDN-2.
    ...
    Examples::
        ...
        >>> o, ht = chunk_qgdn2(q, k, v, g, b, w, output_final_state=True)
    """
